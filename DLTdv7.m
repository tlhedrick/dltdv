...
switch call
  ...
  case {99} % Initialize the GUI
    ...
    fprintf('\n')
    disp('DLTdv7 (updated July 22, 2019)')
    fprintf('\n')
  ...
  case {0} % Initialize button press from the GUI
    ...
    % set some other values to reasonable defaults
    uda.lastvnum=1;
    uda.lastaxh=301;
...
if isempty(filename)==false
  ...
  % change other settings if DLTdv7 was previously in 2D mode
  if uda.dlt==0
    uda.dlt=1; % turn on DLT for the whole interface
    uda.handles(53).Enable='on'; % enable DLT feedback checkbox
    uda.handles(53).Value=1; % set the DLT feedback checkbox to true
    
    s=tsWinMakePDstring(uda); % update timeseries window
    uda.handles(603).String=s;
  end
...
wh.Children.Title.String=['Loading ',pathname,filesep,filename];
